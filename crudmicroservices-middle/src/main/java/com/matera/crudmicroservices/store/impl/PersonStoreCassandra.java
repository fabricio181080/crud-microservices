...
public class PersonStoreCassandra implements PersonStore {
    ...
    private static final DynamicStringProperty KEYSPACE = DynamicPropertyFactory.getInstance().getStringProperty(
        "crudmicroservicesmiddle.cassandra.keyspace", "crudmicroservices");

    private static final DynamicStringProperty CONSISTENCY_LEVEL = DynamicPropertyFactory.getInstance()
        .getStringProperty("crudmicroservicesmiddle.cassandra.concistencylevel", "ONE");

    private static final DynamicStringProperty PERSON_COLUMN_FAMILY = DynamicPropertyFactory.getInstance()
        .getStringProperty("crudmicroservicesmiddle.cassandra.cf.person", "person");

    private static final DynamicStringProperty PERSON_BY_NAME_COLUMN_FAMILY = DynamicPropertyFactory.getInstance()
        .getStringProperty("crudmicroservicesmiddle.cassandra.cf.personbyname", "person_by_name");
    ...
    public void save(Person person) {
        ...
        final Insert insertPerson =
            QueryBuilder.insertInto(KEYSPACE.get(), PERSON_COLUMN_FAMILY.get()).value(ID, person.getId())
                .value(NAME, person.getName()).value(PHONE_NUMBER, person.getPhoneNumber());
        ...
        final Insert insertPersonByName =
            QueryBuilder.insertInto(KEYSPACE.get(), PERSON_BY_NAME_COLUMN_FAMILY.get()).value(ID, person.getId())
                .value(NAME, person.getName()).value(PHONE_NUMBER, person.getPhoneNumber());
    ...
    public Long getMaxId() {
        ...
        logger.info("Querying for max person id");
        final Select query = QueryBuilder.select().from(KEYSPACE.get(), PERSON_BY_NAME_COLUMN_FAMILY.get());

        return new SearchPerson(query, session.get())
            .observe()
            .map(
                persons -> {
